...
namespace Bolt.Automation.Tests.TestExtension.Base
{
    public class UITestBase : TestBase
    {
        ...
        // With LevelOfParallelism(50), all tests try to launch browsers simultaneously.
        // NUnit blocks worker threads via AsyncToSyncAdapter, so async continuations
        // from Playwright can't complete if all threads are occupied.
        ...
        /// <summary>
        /// NUnit runs [SetUp] base-first: TestBase.InitializeTestAsync() (logger + ResolveServices),
        /// then this method (browser + InitializeComponents). By the time the test method runs,
        /// all DI services, browser, and composite helpers are ready.
        /// </summary>
        [SetUp]
        ...
        /// Called during [SetUp] AFTER browser initialization, so BrowserManager, Executor,
        /// and PageFactory are all available. DI services should be resolved in
        /// <see cref="TestBase.ResolveServices"/> instead.
        /// </summary>
        ...
        private async Task EnsureBrowserComponentsInitializedAsync()
        {
            ...
            // Throttle concurrent browser launches to avoid thread pool starvation.
            // With LevelOfParallelism(50) and 5 slots, worst case is 9 queued batches.
            // Each browser launch can take 15-30s on remote CI, so 9 × 30s = 270s max but
            // batches overlap as slots free up, so 180s timeout provides adequate margin.
        ...
        protected async Task UploadEmailArtifactAsync(string? htmlContent)
        {
            if (string.IsNullOrEmpty(htmlContent)) return;
            var fileName = $"email_{DateTime.UtcNow:yyyyMMdd_HHmmss}.html";
            var bytes = System.Text.Encoding.UTF8.GetBytes(htmlContent);
            await UploadArtifactBytesAsync(bytes, fileName, "text/html", ArtifactType.Email);
        }
